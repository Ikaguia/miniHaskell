...
//used when manipulating the pair element
#define ff first
#define ss second

var sumOp	(string& expr);
var subOp	(string& expr);
var getInt 	(string& expr);
var multOp	(string& expr);
var divOp	(string& expr);
var andOp	(string& expr);
var orOp	(string& expr);
var notOp	(string& expr);
var eqOp	(string& expr);
var eqOp	(string& expr);
var gtOp 	(string& expr);
var gtOp 	(string& expr);
var ltOp 	(string& expr);
var ltOp 	(string& expr);
var geOp 	(string& expr);
var geOp 	(string& expr);
var leOp 	(string& expr);
var leOp 	(string& expr);

//Functions that handles calls to prelude operators functions 
...
var let(string& in){//(x=10,y=20)in(...)
	...
	while(1){
		...
		var result=oldParser::runExpr(val);
		if(result.t==error){
	...
	var result = oldParser::runExpr(in);
...
var ifThenElse(string& in){//(a)then(x)else(y)
	...
	var result=oldParser::runExpr(cond);
...
var callFunc(string& func,string& args){
	return funcs[func].call(args, false);
}
...
//Handles each binary's operands expression evaluation
void getBinOperatorVals(string& expr,var& val1,var& val2){
	...
	val1=oldParser::runExpr(s);
	s=expr.substr(middle+1);
	val2=oldParser::runExpr(s);
}
...
var notOp(string& expr){
	var val=oldParser::runExpr(expr);
	if(val.t==error)return val;
	if(val.t!=tBool)return var();
	return var(!val.val);
}

var oldParser::runExpr(string& expr){
